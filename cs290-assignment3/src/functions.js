...
var bar = function(doubleArray){
	var number;
	for (number = 0; number < doubleArray.length; number++){
		//http://www.w3schools.com/jsref/jsref_isnan.asp
		if(isNaN(doubleArray[number])=== true){
			return false;
		}
		else{
			doubleArray[number]= 2*doubleArray[number];
		}
	}
	return true;
}
...
* 3782618 Wed, 7 Jan 2015 21:42:26 -0800 "Initial *
* |hash | | date | | message |
commit"
...
function parseGit(logArray){
	...
	for (var i = 0; i < logArray.length; i++){
		//space between the hash and date, so we need to find index of 1st space
		var dateIndex = logArray[i].indexOf(' ');
		//next we will find the index of '"' where the message starts
		var messageStartIndex = logArray[i].indexOf('"');
		//then will find there index of where the message ends
		var messageEndIndex = logArray[i].lastIndexOf('"');
		//now we can put everything together
		gitLog[i] = new GitLog(logArray[i].substring(0, dateIndex), new Date(logArray[i].substring(dateIndex, messageStartIndex)), logArray[i].substring(messageStartIndex +1, messageEndIndex));
	}
	return gitLog;
}
//end your code